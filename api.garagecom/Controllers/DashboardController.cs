...
public class DashboardController : Controller
{
    ...
    public async Task<ApiResponse> GetDashboardSigns(IFormFile file)
    {
        ...
        List<string> defects = await AiHelper.GetDashboardSigns(file);
        List<Defects> defectsList = new List<Defects>();
        var sql = @"SELECT Logo, Title, Description, Solution FROM DashboardSigns DS WHERE LOWER(Title) = LOWER(@Defect)";
            ...
            using (var reader = DatabaseHelper.ExecuteReader(sql, parameters))
            {
                while (reader.Read())
                {
                    defectsList.Add(new Defects()
                    {
                        ...
                        Solution = (reader["Solution"] == DBNull.Value ? "" : reader["Solution"].ToString()) ?? string.Empty
                    });
                }
            }
        }
